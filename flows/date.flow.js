...
const confirmationFlow = addKeyword(EVENTS.ACTION)
  .addAnswer("Confirmas la fecha propuesta? Responde unicamente con 'si' o 'no'", { capture: true }, async (ctx, ctxFn) => {
    if (ctx.body.toLowerCase().includes("si")) {
      return ctxFn.gotoFlow(formFlow)
    } else {
      await ctxFn.endFlow("Reserva cancelada. Volve a solicitar una reserva para elegir otra fecha.");
    }
  });
...
const dateFlow = addKeyword(EVENTS.ACTION)
  ...
  .addAnswer("Revisando disponibilidad...", null, async (ctx, ctxFn) => {
    ...
    if (!dateAvailable) {
        ...
        const response = await chat(promptBase + `\nHoy es el dia: ` + currentDate + `\nLa fecha solicitada es ` + solicitedDate + "\nLa disponibilidad de esa fecha es: false. El proximo espacio disponible que tenes que ofrecer es: " + dateText + "Da la fecha siempre en español", messages);
